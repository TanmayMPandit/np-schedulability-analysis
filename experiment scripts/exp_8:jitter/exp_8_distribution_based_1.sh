...
jobset_dir="exp_8/${ratio}/rand-fixed-sum-utilDist/log-uniform-discrete-perDist/4-core/6-task/100-jitter/1.60-util/jobsets"
    ...
    if [[ -d "$jobset_dir" ]]; then
        ...
        for jobset_file in "$jobset_dir"/*; do
            ...
            if [[ -f "$jobset_file" ]]; then
                ...
                (build/nptest "$jobset_file" -m 4 -f "0.74,0.8,0.87,0.94,1.0"   --energy-timeout 3600 -o "results/exp_8" -u "$ratio") &
            else
