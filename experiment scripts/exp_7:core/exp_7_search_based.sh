    ...
    if [[ -d "$jobset_dir" ]]; then
        ...
        for jobset_file in "$jobset_dir"/*; do
            ...
            if [[ -f "$jobset_file" ]]; then
                ...
                (build/nptest "$jobset_file" -m $core_value -f "0.74,0.8,0.87,0.94,1.0" --search-based --link_threshold 50 -k 1 -b 0 --search_threshold 100   --energy-timeout 3600 -o "results/exp_7" -u "$core_value") &
            else
