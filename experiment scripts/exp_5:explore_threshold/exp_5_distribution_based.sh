...
for limit in "${explore_limits[@]}"; do
    ...
    if [[ -d "$jobset_dir" ]]; then
        ...
        for jobset_file in "$jobset_dir"/*; do
            ...
            if [[ -f "$jobset_file" ]]; then
                ...
                (build/nptest "$jobset_file" -m 4 -f "0.74,0.8,0.87,0.94,1.0" --link_threshold $limit -k 1 -b 0 --search_threshold 100   --energy-timeout 3600 -o "results/exp_5" -u "$limit") &
            else
